...
* `bash_dev_ignore_styles` - style ids exempt from the dev and identical-strafe detections. Unlike `bash_bypass` this keeps gain logging on, so an autosync style can still be logged without endless dev logs.
